...
from src.features.process_text.normalize import expand_contractions, remove_special_characters,\
    ...
    replace_apostrophes, replace_multiple_stopwords, remove_numbers, expand_abbreviations, emoticon_convert
...
def clean_text(text, tokenize=False):
    ...
    clean_dict['expand emoticons'] = emoticon_convert(clean_dict['replace_whitespaces'])
    # Replace multiple stopwords.
    clean_dict['replace_multiple_stopwords'] = replace_multiple_stopwords(clean_dict['expand emoticons'])
    # Replace apostrophes.
    ...
    if not clean_dict['sentence_tokenize']:
        ...
        return clean_dict, clean_dict['remove_stopwords']